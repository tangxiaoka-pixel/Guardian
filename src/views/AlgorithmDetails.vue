<template>
  <section class="page" v-loading="loading">
    ...
    <template v-if="detail?.scenario === 'desk_drink_intrusion'">
      ...
      <el-card class="panel" shadow="never"><template #header>线上 Qwen 审计提示词</template>
        <div class="prompt-meta">当前模型：<code>{{ detail?.teacher_model || 'ollama/qwen2.5vl:7b@5070Ti' }}</code>；输出坐标：源图像像素坐标（1280 × 720）。</div>
        <pre class="prompt">{{ detail?.vlm_runtime_prompt || detail?.vlm_audit_prompt || '-' }}</pre>
      </el-card>
