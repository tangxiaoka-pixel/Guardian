...
router.beforeEach((to) => {
  ...
  const platformForgePaths = ['/forge/centers', '/forge/activation', '/forge/project-bindings', '/forge/device-bindings', '/forge/sample-policies', '/forge/model-versions', '/forge/release-approvals', '/forge/releases', '/forge/heartbeats', '/forge/sync-logs']
  const platformPaths = ['/dashboard', '/customer-center', '/audit', '/role-matrix', '/platform-edge-gateways', '/devices-logs', '/capacity-planner', '/algorithm-details', '/closed-loop-trace', '/scenario-policies', '/model-registry', ...platformForgePaths]
  const projectManagerPaths = ['/customer-workspace', '/edge-gateways', '/streams', '/camera-bindings', '/runtime-configs', '/l1-monitor', '/l2-monitor', '/alarm-center', '/event-handling', '/device-status', '/project-settings', '/human-review', '/core-logs', '/ai-lifecycle', '/forge', '/forge/materials', '/forge/vlm-audit', '/forge/human-review', '/forge/datasets', '/forge/release']
  ...
  const isTrainingCenterPath = (to.path.startsWith('/forge') && !platformForgePaths.includes(to.path)) || trainingCenterPaths.includes(to.path)
